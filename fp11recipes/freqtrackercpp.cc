...
struct FreqTracker
{
  ...
  // We record trajectories by nesting hashed associative
  // containers. The outer hash is on origin time.  The inner
  // is on position. Finally, the pair is effect size + the
  ...
  inline void operator()(const fwdpy11::DiploidPopulation& pop,
        ...
        if (pop.mcounts[i] > 0 && pop.mcounts[i] <= 2 * pop.N) {
          ...
          if (origin_itr == end(trajectories)) {
            trajectories[origin][pop.mutations[i].pos] =
              std::make_pair(pop.mutations[i].s, trajectory{pop.mcounts[i]});
          } else {
            auto pos_itr = origin_itr->second.find(pop.mutations[i].pos);
            if (pos_itr == end(origin_itr->second)) {
              origin_itr->second[pop.mutations[i].pos] =
                std::make_pair(pop.mutations[i].s, trajectory{pop.mcounts[i]});
            } else {
              if (pos_itr->second.second.back() < 2 * pop.N) {
                pos_itr->second.second.push_back(pop.mcounts[i]);
              }
            }
          }
        }
  ...
  py::dict get_trajectories() const
  {
    py::dict rv;
    ...
    for (auto&& origin : trajectories) {
      for (auto&& position : origin.second) {
        // Make a Python tuple
        auto key =
          py::make_tuple(origin.first, position.first, position.second.first);
...
PYBIND11_MODULE(freqtrackercpp, m)
{
  py::class_<FreqTracker>(m, "FreqTracker")
    .def(py::init<std::uint32_t>(),
         py::arg("burnin_time")) // This is __init__
    .def("__call__",
         &FreqTracker::operator()) // This makes it a valid
                                   // Python callable
    .def_property_readonly("trajectories",
                           &FreqTracker::get_trajectories); // Get the data as a
                                                            // dict in Python.
}
